...
% end
if ~isfield(problem, 'costgrad') && ~isfield(problem, 'egrad')
    %---> Example: "Karcher mean" code 
    VTFreeCholesky_flag = true;  %--> it means Riemannian gradient is given directly by the user (problem) and not computed from Euclidean gradient. So, we do not implicitly map the Riemannian gradient in egrad2rgrad operator of manifold_VTF and should map it explicitly in the algorithm.
else
    %---> Example: "RiemMix" (Gaussian mixture model) code, "MetricLearning" code
    VTFreeCholesky_flag = false;  %--> it means Riemannian gradient is not given directly by the user (problem) and is computed from Euclidean gradient. So, we do implicitly map the Riemannian gradient in egrad2rgrad operator of manifold_VTF (in the world of mapping) and should not map it explicitly in the algorithm again.
    %--> [Note that the Euclidean gradient may have been given by user, but
    %not the Riemannian gradient. In that case, Riemannian gradient is still calculated implicitely by the manifold operators so it is in the world of mapping. Hence, in that case, there is no need to map the gradient explicitely, too]
end
...
while true
    ...
    if isfield(lsmem,'newd') && ~isempty(lsmem.newd)  %----> Benyamin added the condition ~isempty(lsmem.newd)
...
info = info(1:iter+1);
%fprintf('costGradexec = ',costGradexec);
