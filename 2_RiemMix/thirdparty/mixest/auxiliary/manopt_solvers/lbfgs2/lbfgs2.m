...
function [x cost info] = lbfgs2(problem, x, options)
% Manifold LBFGS minimization algorithm for Manopt.
%
...
localdefaults.linesearch = @linesearch_wolfe; %linesearch
costevals = 1;
...
while true
    ...
    elseif true
        % This is worse!
        if isfield(problem.M,'transpf')
            [gradC, Expc, Expci] = problem.M.transpf(newx, x, newgrad);
        else
            gradC = problem.M.transp(newx, x, newgrad);
        end
        grad_diff = problem.M.lincomb(x, 1, gradC, -1, grad);
        
        desc_dir_step = problem.M.lincomb(x, stepsize, desc_dir);
    else
        grad_diff = problem.M.lincomb(x, 1, newgrad, -1, grad);
    ...
    if isfield(problem.M,'transpf')
        [grad_diff_all, desc_dir_all, x_all, gd_all, ddgd_all, H, ...
            Expc_all, Expci_all] = ...
            lbfgs_update2(newx, problem.M, grad_diff, desc_dir_step, ...
            grad_diff_all, desc_dir_all, options.numgrad, x_all, gd_all, ...
            ddgd_all, H, options, Expc, Expci, Expc_all, Expci_all);
    else
        [grad_diff_all, desc_dir_all, x_all, gd_all, ddgd_all, H] = ...
            lbfgs_update2(newx, problem.M, grad_diff, desc_dir_step, ...
            grad_diff_all, desc_dir_all, options.numgrad, x_all, gd_all, ...
    ...
    if stats.curiterratio < 1 % This is NOT the last iteration in a mini-batch
        ...
        elseif isfield(problem.M,'transpf')
            desc_dir = desc_dir_cal2(newgrad, problem.M, grad_diff_all, ...
                desc_dir_all, x_all, ddgd_all, gd_all, length(gd_all) , H, ...
                Expc_all, Expci_all);
        else
            desc_dir = desc_dir_cal2(newgrad, problem.M, grad_diff_all, ...
