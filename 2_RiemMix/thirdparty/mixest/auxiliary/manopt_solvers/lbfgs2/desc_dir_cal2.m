...
function  p_desc = desc_dir_cal2(p, M, grad_diff_all, desc_dir_all, ...
...
if isfield(M,'transpf')
    if corrections >1
        vec_new= desc_dir_cal(p_prev, M, grad_diff_all, desc_dir_all, ...
            x_all, ddgd, gd, corrections-1, Hdiag, Expc_all, Expci_all);
    else
        vec_new = M.lincomb(x_all{corrections+1}, Hdiag, p_prev);
    end
else
    if corrections >1
        vec_new= desc_dir_cal(p_prev, M, grad_diff_all, desc_dir_all, ...
