%% |splitTmerge|
% splitTmerge algorithm for estimation of mixture model structure and
...
% *Description*
%
% This function implements the splitTmerge (split-then-merge) algorithm
% proposed by Hosseini (2012), though here we use generalized
% optimization methods.
%
% |[theta, D] = splitTmerge(data, target_num)| returns the estimated
% parameters and the mixture distribution |D| with |target_num| components,
% fitted to |data|.
%
% |[theta, D] = splitTmerge(data, target_num, options)| utilizes applicable
% options from the |options| structure in the estimation procedure.
%
% |[theta, D, info] = splitTmerge(...)| also returns |info|, a structure
% array containing information about successive iterations performed by
% iterative estimation functions.
%
% |[theta, D, info, options] = splitTmerge(...)| also returns the effective
...
%   [theta, D] = splitTmerge(data, 5, options)
%
% *References*
%
% # R. Hosseini, Statistical modeling of natural images using mixture 
% models with compression as an application, PhD thesis, TU Berlin, 2012.
%
...
function [theta, D, info, options] = splitTmerge(data, target_num, options)
% This function implements the Total-Split method 
%
% This procedure applies splitting approach (partial splitting to increase
% the speed). Then based on history of improvements an initialization 
% is used
%
    ...
    layvec = 0; % The layer of placement of each component
...
%                 pause
    ...
    % Final estimation
    if options.verbosity >= 1
        fprintf('\nPerforming the final estimation for the mixture with %d components...\n', D.num())
    end
    % No initialization!! here
    opt.theta0 = theta;
    [theta, D] = D.estimate(data, opt);
