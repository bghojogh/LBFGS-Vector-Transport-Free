...
fprintf(fid, 'Method \t itr=%d \t itr=%d \t itr=last \t n_iters \t time \n', iterations_to_report{1}, iterations_to_report{2});
fprintf(fid, '=============================================== \n');
...
for method_index = 1:length(methods)
    ...
    fprintf(fid, '%s \t %.3f|+|%.3f \t %.3f|+|%.3f \t %.3f|+|%.3f \t %.3f|+|%.3f \t %.3f|+|%.3f \n', methods{method_index}, mean_cost(1), std_cost(1), mean_cost(2), std_cost(2), mean_cost(3), std_cost(3), n_itr_mean, n_itr_std, time_mean, time_std);
end
fclose(fid);
